...
def flush() -> None:
    """Долгоживущей команде надо выталкивать вывод самой.

    При выводе не в терминал Python буферизует stdout, и адрес страницы,
    напечатанный один раз в начале, доходит только при завершении процесса —
    то есть никогда, если команда работает часами.
    """
    sys.stdout.flush()
    sys.stderr.flush()
