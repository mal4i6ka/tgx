...
def webapp_dom_regression() -> None:
    ...
    # Скрытое в вывод попадать не должно. Сырой textContent берёт и его: на этом
    # я обжёгся, приняв всегда присутствующий, но невидимый блок «поверни
    # телефон» за состояние приложения — и пять раз искал несуществующую
    # причину. Инструмент отвечал верно, ошибалась проба.
    check("видимость проверяется у каждого узла", "if (!visible(node)) return;" in page)
