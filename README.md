...
**Что проверено.** Wallet, Epic Gift, Gorilla Case, Random Games, TonMobile eSIM
и Тюряга — все запускаются и работают: баланс, разделы, вкладки, живые данные.
Тюряга просит альбомную ориентацию — это её условие, а не поломка.
...
**Чего это не решает.** Игра «Тюряга» просит альбомную ориентацию и продолжает
просить её при `--as android --phone`: платформа android, касания 5, угол 90,
рамка 1280×534 — альбомная и по медиазапросу тоже. Я проверил все признаки,
какие смог назвать, и ни один не оказался решающим; условие лежит в
подгружаемом куске, до которого я не добрался. Так что переключатель полезен
сам по себе, но эту игру он не открывает — и делать вид, что открывает, не
стоит.
