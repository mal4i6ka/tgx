...
Чего здесь нет намеренно: **вывод средств** (`getStarsRevenueWithdrawalUrl`
требует пароль от аккаунта — его место не в аргументах командной строки) и
**поиск банка по номеру карты** (номер карты не должен попадать в историю
команд и логи).
