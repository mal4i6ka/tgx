...
**Ограничение, которое не обойти.** Приложение вправе объявить, кто может
держать его в рамке. Wallet, например, пускает только `web.telegram.org` и себя
самого; запрет исполняет браузер. Такое приложение в окне не откроется, и `tgx`
это показывает словами вместо белого прямоугольника, предлагая открыть вкладкой
— там оно запустится, но без хозяина.

Признак один — тишина: настоящее приложение здоровается первым делом, поэтому
молчание дольше пяти секунд считается отказом.
